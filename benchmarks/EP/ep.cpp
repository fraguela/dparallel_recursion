...
/* from main locals to globals, for convenience */
double Mops, t1, t2, sx, sy, tm, an, gc;
...
tbb::enumerable_thread_specific<std::array<double, 2*NK>> global_x;

/*--------------------------------------------------------------------
	parallel_recursion classes
 --------------------------------------------------------------------*/
...
Result result_sum(const Result& othera, const Result& otherb)
{
  Result r {othera.sx + otherb.sx, othera.sy + otherb.sy};
  std::transform(othera.q, othera.q + NQ, otherb.q, r.q, std::plus<double>());
  return r;
}

BOOST_IS_BITWISE_SERIALIZABLE(Result);
/*--------------------------------------------------------------------
 program EMBAR
 c-------------------------------------------------------------------*/
/*
...
int main(int argc, char **argv)
{
  ...
  Result res;
  dpr_pfor_reduce(k, 1, np + 1, nthreads * tasks_per_thread, res, Result{}, result_sum,
                  { double t1, t2, t3, t4, x1, x2;
                    int kk, i, ik, l;
                    double * const x = global_x.local().data();
  ...
  sx = res.sx;
  sy = res.sy;
  // END #pragma omp parallel copyin(x)
  timer_stop(1);
  tm = timer_read(1);
  
  double qq[NQ];    /* sum of all q */
  //MPI_Reduce(q, qq, NQ, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  for (i = 0; i < NQ; i++) {
    qq[i] = res.q[i];
    gc = gc + qq[i];
  }
  ...
  if (M == 24) {
    ...
    if ((fabs((sx - (1.982481200946593e5)) / sx) <= EPSILON) &&
        (fabs((sy - (-1.020596636361769e5)) / sy) <= EPSILON)) {
      verified = TRUE;
    }
  } else if (M == 40) {
    if ((fabs((sx - (-5.319717441530e5)) / sx) <= EPSILON) &&
        (fabs((sy - (-3.688834557731e5)) / sy) <= EPSILON)) {
      verified = TRUE;
    }
  }
  ...
  if(!rank) {
    ...
    c_print_results("EP", CLASS, M + 1, 0, 0, 0, nprocs * nthreads,
                    tm, Mops,
