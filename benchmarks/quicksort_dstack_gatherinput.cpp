...
///
/// \file     quicksort_dstack_gatherinput.cpp
...
int main(int argc, char **argv) {
	...
	if (!runTests) {
		if (chunkSize > 0) {
			...
			if (partitioner == 1) {
				...
				dpr::dparallel_stack_recursion<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::custom(), dpr::GatherInput);
			} else if (partitioner == 2) {
				//_partitioner = 2 => automatic
				dpr::dparallel_stack_recursion<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::automatic(), dpr::GatherInput);
			} else {
				//_partitioner = 0 => simple
				dpr::dparallel_stack_recursion<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::simple(), dpr::GatherInput);
			}
		} else {
			srand(1234);
			...
			if (partitioner == 1) {
				//_partitioner = 1 => custom
				dpr::dparallel_stack_recursion<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::custom(), dpr::GatherInput, dpr::dspar_config_info_default, opt, tmp_test);
			} else if (partitioner == 2) {
				//_partitioner = 2 => automatic
				dpr::dparallel_stack_recursion<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::automatic(), dpr::GatherInput, dpr::dspar_config_info_default, opt, tmp_test);
			} else {
				//_partitioner = 0 => simple
				dpr::dparallel_stack_recursion<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::simple(), dpr::GatherInput, dpr::dspar_config_info_default, opt, tmp_test);
			}
		}
	} else {
		gettimeofday(&t0, NULL);
		if (partitioner == 1) {
			//_partitioner = 1 => custom
			listChunks = dpr::dparallel_stack_recursion_test<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::custom(), dpr::GatherInput, dpr::dspar_config_info_default, opt);
		} else if (partitioner == 2) {
			//_partitioner = 2 => automatic
			listChunks = dpr::dparallel_stack_recursion_test<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::automatic(), dpr::GatherInput, dpr::dspar_config_info_default, opt);
		} else {
			//_partitioner = 0 => simple
			listChunks = dpr::dparallel_stack_recursion_test<void> (tmp, qsinfo, QS(), chunkSize, dpr::partitioner::simple(), dpr::GatherInput, dpr::dspar_config_info_default, opt);
		}
	}
	...
	if (rank == 0) {
		...
		if (!runTests) {
			...
			if (checkOrder) {
				std::cout << "[" << rank << "] check results: *SUCCESS*" << std::endl;
			} else {
				std::cout << "[" << rank << "] check results: FAILURE!" << std::endl;
			}
#endif
		} else {
