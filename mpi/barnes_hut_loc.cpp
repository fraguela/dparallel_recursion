...
/// Easy to parallelize but probably not too worth
static inline void ComputeCenterAndDiameter(const int n, double &diameter, double &centerx, double &centery, double &centerz)
{
  ...
  for (int i = 0; i < n; i++) {
    posx = Actual_bodies[i]./*body[i]->*/posx;
    posy = Actual_bodies[i]./*body[i]->*/posy;
    posz = Actual_bodies[i]./*body[i]->*/posz;

    if (minx > posx) minx = posx;
    if (miny > posy) miny = posy;
    if (minz > posz) minz = posz;

    if (maxx < posx) maxx = posx;
    if (maxy < posy) maxy = posy;
    if (maxz < posz) maxz = posz;
  }

  diameter = maxx - minx;
  if (diameter < (maxy - miny)) diameter = (maxy - miny);
  if (diameter < (maxz - minz)) diameter = (maxz - minz);
...
int main(int argc, char *argv[])
{
  ...
  if (rank == 0) {
    ...
    if (argc < 2) {
      fprintf(stderr, "\narguments: filename [nreps] [output]\n");
      exit(-1);
    }
  }
  ...
  //while (((run < 3) || (labs(lasttime-runtime)*64 > std::min(lasttime, runtime))) && (run < 7)) {
    ...
    for (step = 0; step < timesteps; step++) {  // time-step the system
      ...
      //printf("%lf %lf %lf %lf (%lf)\n", root->mass, root->posx, root->posy, root->posz, diameter);
