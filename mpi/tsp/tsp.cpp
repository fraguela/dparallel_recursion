...
void recv(int from_rank, int tag, Tree& data, MPI_Status& st)
{ int len;
 
  MPI_Probe(from_rank, tag, MPI_COMM_WORLD, &st);
  from_rank = st.MPI_SOURCE;
  tag = st.MPI_TAG;
...
//partition_row_count could have also been reused with an extern declaration
int tsp_partition_row_count = 0;

Tree tsp_top(Tree t, int sz) {
  Tree leftval, rightval;
  
  //assumes no conquer at this level
  if ((t->sz / 2) == bottom_n ) {
    //printf("Pick bottom [%p]\n", t);
    leftval  = partition_row[tsp_partition_row_count++];
    rightval = partition_row[tsp_partition_row_count++];
  } else {
    //printf("Pick children [%p]\n", t);
    leftval = tsp_top(t->left, sz);
    rightval = tsp_top(t->right, sz);
  }
  return merge(leftval, rightval, t);
}
...
Tree tsp(int rank, int nprocs, Tree t, int sz) {
  ...
  if (rank) {
    ...
    for (int i = 0; i < (nleaves - myleaves); i++) {
      recv(MPI_ANY_SOURCE, MPI_ANY_TAG, tmp, st);
      //print_list(tmp);
      //printf("setting %d\n", st.MPI_TAG);
      partition_row[st.MPI_TAG] = tmp;
    }
