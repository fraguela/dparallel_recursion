...
int nleaves, bottom_n, bottom_n_copy, myleaves, myfirst_leaf, partition_row_count = 0;
Tree *partition_row;
...
Tree build_tree(int n, int dir, double min_x, double max_x, double min_y, double max_y) {
  ...
  if (n == bottom_n) {
    if (IS_TOP_PART) {
      if((partition_row_count >= myfirst_leaf) && (partition_row_count < (myfirst_leaf + myleaves))) {
        bottom_n = 0;
        t = build_tree<false>(n, dir, min_x, max_x, min_y, max_y);
  ...
  if (dir) {
    ...
    t->left = build_tree<IS_TOP_PART>(n / 2, dir, min_x, med, min_y, max_y);
    t->right = build_tree<IS_TOP_PART>(n / 2, dir, med, max_x, min_y, max_y);
  } else {
    dir = !dir;
    ...
    t->left = build_tree<IS_TOP_PART>(n / 2, dir, min_x, max_x, min_y, med);
    t->right = build_tree<IS_TOP_PART>(n / 2, dir, min_x, max_x, med, max_y);
  }
  t->sz = n;
  t->next = NULL;
  t->prev = NULL;
...
Tree build_tree(int rank, int nprocs, int n, int dir, double min_x, double max_x, double min_y, double max_y) {
  ...
  return build_tree<true>(n, dir, min_x, max_x, min_y, max_y);
}
