...
int treeadd_rec(tree_t* root) {
	if(!root->left)
		...
		return treeadd_rec(root->left) + treeadd_rec(root->right) + root->val;
}

int treeadd_mpi(tree_t* root, int rank, int nprocs) {
  int result, local_result = 0;
  
  if(root!= NULL) local_result = treeadd_rec(root);
  for (int i = 0; i < myleaves; i++) {
    local_result += treeadd_rec(partition_row[i]);
  }
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < iters; i++) {
        ...
        //MPI_Barrier(MPI_COMM_WORLD); //Not necessary thanks to MPI_Allreduce
	gettimeofday(&t1, NULL);
	timersub(&t1, &t0, &t);
