...
int rank, nprocs;
int bodies_per_rank, first_body, last_body;
...
{
  int step = (n + nwanted_tasks - 1) / nwanted_tasks;

  a6_t res, v[nwanted_tasks * 2]; // The *2 is to avoid false sharing
  
  _Cilk_for(int j = 0; j < n; j+=step) {
    int p = (j /step) * 2;        // The *2 is to avoid false sharing
    v[p] = a6_t{1.0E90, 1.0E90, 1.0E90, -1.0E90, -1.0E90, -1.0E90};
    int limit = std::min(j + step, n);
    for (int i = j; i < limit; i++) {
      v[p] = myreduce(v[p], {Actual_bodies[i].posx, Actual_bodies[i].posy, Actual_bodies[i].posz, Actual_bodies[i].posx, Actual_bodies[i].posy, Actual_bodies[i].posz});
    }
  }
  
  res = v[0];
  for(int i = 2; i < nwanted_tasks * 2; i+=2) {
    res = myreduce(res, v[i]);
  }
...
int main(int argc, char *argv[])
{
  ...
  //while (((run < 3) || (labs(lasttime-runtime)*64 > std::min(lasttime, runtime))) && (run < 7)) {
    ...
    bodies_per_rank = ceil(nbodies / (float)nprocs);
    first_body = bodies_per_rank * rank;
    last_body = std::min(nbodies, first_body + bodies_per_rank);
    OctTreeLeafNode * const mynodes = new OctTreeLeafNode[bodies_per_rank];
    ...
    for (step = 0; step < timesteps; step++) {  // time-step the system
      ...
      _Cilk_for(int i = first_body; i < last_body; i++) {  // the iterations are independent
