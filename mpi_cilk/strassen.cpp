...
void Strassen_seq(const MMatrix& a, const MMatrix& b, MMatrix& c);

MMatrix Strassen(const MMatrix& a, const MMatrix& b)
{
  MMatrix result(a.rows(), b.cols());
  
  Strassen_seq(a, b, result);
...
void Strassen_seq(const MMatrix& a, const MMatrix& b, MMatrix& c)
{
  ...
  if (nl <= base_case_sz) {
    ...
    _Cilk_spawn [&]{ M[0] =  Strassen(A11 + A22, B11 + B22); }();
    _Cilk_spawn [&]{ M[1] =  Strassen(A21 + A22, B11); }();
    _Cilk_spawn [&]{ M[2] =  Strassen(A11, B12 - B22); }();
    _Cilk_spawn [&]{ M[3] =  Strassen(A22, B21 - B11); }();
    _Cilk_spawn [&]{ M[4] =  Strassen(A11 + A12, B22); }();
    _Cilk_spawn [&]{ M[5] =  Strassen(A21 - A11, B11 + B12); }();
...
{ std::vector<Datatree_level_t> datatree;
  ...
  _Cilk_for(int i = 0; i < dist.nLocalElems(); i++) {
      datatree.back()[i].Strassen(); //Deallocates a and b
  }
