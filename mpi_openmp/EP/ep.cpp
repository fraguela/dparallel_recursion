...
/* global variables */
/* common /storage/ */
static double x_storage[2*NK];
#pragma omp threadprivate(x_storage)
static double q[NQ];
...
#pragma omp parallel
  { double qq[NQ];		/* private copy of q[0:NQ-1] */
    ...
    for (int i = 0; i < NQ; i++) qq[i] = 0.0;

    double * x = x_storage;
    
    int iters_per_proc = (np + nprocs -1) / nprocs;
    int end_iter = std::min(np, (rank + 1) * iters_per_proc);
    int chunksz = (iters_per_proc + nthreads * tasks_per_thread - 1) / (nthreads * tasks_per_thread);
    
#pragma omp for reduction(+:sx,sy) schedule(dynamic, chunksz)
...
#pragma omp critical
    {
      ...
      std::transform(q, q + NQ, qq, q, std::plus<double>());
    }
  ...
  MPI_Reduce(q,     gq, NQ, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
