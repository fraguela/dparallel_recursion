...
#pragma omp task default(shared) firstprivate(ntasks) untied
      M[0] = Strassen(A11 + A22, B11 + B22, ntasks);
#pragma omp task default(shared) firstprivate(ntasks) untied
      M[1] = Strassen(A21 + A22, B11, ntasks);
#pragma omp task default(shared) firstprivate(ntasks) untied
      M[2] = Strassen(A11, B12 - B22, ntasks);
#pragma omp task default(shared) firstprivate(ntasks) untied
      M[3] = Strassen(A22, B21 - B11, ntasks);
#pragma omp task default(shared) firstprivate(ntasks) untied
      M[4] = Strassen(A11 + A12, B22, ntasks);
#pragma omp task default(shared) firstprivate(ntasks) untied
      M[5] = Strassen(A21 - A11, B11 + B12, ntasks);
...
#pragma omp parallel for firstprivate(n2) if(n2 >= 512)
    for (int i = 0; i < n2; i++) {
      for (int j = 0; j < n2; j++) {
...
#pragma omp parallel sections
    {
#pragma omp section
      M[0] = Problem(A11 + A22, B11 + B22);
#pragma omp section
      M[1] = Problem(A21 + A22, B11);
#pragma omp section
      M[2] = Problem(A11, B12 - B22);
#pragma omp section
      M[3] = Problem(A22, B21 - B11);
#pragma omp section
      M[4] = Problem(A11 + A12, B22);
#pragma omp section
      M[5] = Problem(A21 - A11, B11 + B12);
#pragma omp section
...
#pragma omp task shared(datatree) firstprivate(i, probl_per_node, step) untied
