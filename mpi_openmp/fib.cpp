...
size_t rec_fib_l(int n, int l) {
  if ( (n < 2) || ( ((1 << l) * probl_per_node) >= nwanted_tasks ) ) {
    return rec_fib(n);
  } else {
      size_t n1, n2;
#pragma omp task shared(n1) firstprivate(n,l) untied
      n1 = rec_fib_l(n - 1, l + 1);
      n2 = rec_fib_l(n - 2, l + 1);
#pragma omp taskwait
      return n1 + n2;
  }
}
...
#pragma omp parallel
#pragma omp single
  for (int i = begin; i < limit; i += step) {
#pragma omp task firstprivate(i, begin, limit, step) shared(result_arr,cur_level) untied
    for (int j = i; j < std::min(i + step, limit); j++) {
      result_arr[j-begin] = rec_fib_l((*cur_level)[j], 0);
    }
