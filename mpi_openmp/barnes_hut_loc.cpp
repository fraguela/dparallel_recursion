...
#pragma omp parallel for reduction(min: minx, miny, minz) reduction(max: maxx, maxy, maxz)
  for (int i = 0; i < n; i++) {
    double posx = Actual_bodies[i]./*body[i]->*/posx;
    double posy = Actual_bodies[i]./*body[i]->*/posy;
    double posz = Actual_bodies[i]./*body[i]->*/posz;
...
int main(int argc, char *argv[])
{
  ...
  const int nwanted_tasks = nthreads * tasks_per_thread;
