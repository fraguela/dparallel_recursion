...
#pragma omp task firstprivate(buf, len, tag, chunk_size) shared(partition_row) untied
  {
    len /= 2;
    Tree data = new tree[len];
  
    for(int j=0; j < len; j+=chunk_size)
#pragma omp task shared(buf, data) firstprivate(j,len) untied
    {
