...
int nleaves, bottom_n, bottom_n_copy, myleaves, myfirst_leaf, partition_row_count = 0, par_level;
Tree *partition_row;
...
template<bool IS_TOP_PART, bool IS_PARALLEL_PART>
Tree build_tree(int n, int dir, double min_x, double max_x, double min_y, double max_y) {
  ...
  if (n == bottom_n) {
    if (IS_TOP_PART) {
      if((partition_row_count >= myfirst_leaf) && (partition_row_count < (myfirst_leaf + myleaves))) {
        bottom_n = 0;
        t = build_tree<false, true>(n, dir, min_x, max_x, min_y, max_y);
        bottom_n = bottom_n_copy;
      } else {
        t = NULL;
      }
  ...
  if (dir) {
    ...
    if (IS_PARALLEL_PART && (n > par_level)) {
#pragma omp task default(shared) untied
      t->left = build_tree<IS_TOP_PART, true>(n / 2, dir, min_x, med, min_y, max_y);
      t->right = build_tree<IS_TOP_PART, true>(n / 2, dir, med, max_x, min_y, max_y);
#pragma omp taskwait
    } else {
      t->left = build_tree<IS_TOP_PART, false>(n / 2, dir, min_x, med, min_y, max_y);
      t->right = build_tree<IS_TOP_PART, false>(n / 2, dir, med, max_x, min_y, max_y);
    }
  } else {
    dir = !dir;
    ...
    if (IS_PARALLEL_PART && (n > par_level)) {
#pragma omp task default(shared) untied
      t->left = build_tree<IS_TOP_PART, true>(n / 2, dir, min_x, max_x, min_y, med);
      t->right = build_tree<IS_TOP_PART, true>(n / 2, dir, min_x, max_x, med, max_y);
#pragma omp taskwait
    } else {
      t->left = build_tree<IS_TOP_PART, false>(n / 2, dir, min_x, max_x, min_y, med);
      t->right = build_tree<IS_TOP_PART, false>(n / 2, dir, min_x, max_x, med, max_y);
    }
  }
  t->sz = n;
  t->next = NULL;
...
#pragma omp parallel
#pragma omp single
  ret = build_tree<true, false>(n, dir, min_x, max_x, min_y, max_y);
